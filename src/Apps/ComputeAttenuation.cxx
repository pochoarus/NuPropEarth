...
double          gOptRadius = 0.; //point source
...
int main(int argc, char** argv)
{
  ...
  IncomingFlux * flx_driver = new IncomingFlux(gOptPdg, gOptAlpha, gOptCthmin, gOptCthmax, gOptCthmono, gOptEmin, gOptEmax, gOptEmono, gOptRadius);
  ...
  OutEvFile += Form("_r%g",gOptRadius);
    ...
    if (event) {
      ...
      while ( (p=(GHepParticle *)piter.Next()) ) {
        if ( pdg::IsNeutrino(TMath::Abs(p->Pdg())) && p->Status() && p->E()>gOptEmin ) {
          ...
          p->SetEnergy( TMath::Sqrt(p->Px()*p->Px()+p->Py()*p->Py()+p->Pz()*p->Pz()+Tauola::getTauMass()*Tauola::getTauMass()) ); //use this energy to avoid energy conservation in tauola
          p->SetPosition( X4.X()+p->Vx()*1e-15, X4.Y()+p->Vy()*1e-15, X4.Z()+p->Vz()*1e-15, X4.T()+p->Vt() ); //position -> passing from fm [genie] to m
...
vector<GHepParticle> DecayTau(GHepParticle * tau) {
  ...
  LOG("ComputeAttenuation", pDEBUG) << "DecayTau: " << tau->Pdg() << ", E = " << tau->E();
  LOG("ComputeAttenuation", pDEBUG) << "  Position   = [ " << tau->Vx() << " , " << tau->Vy() << " , " << tau->Vz() << " , " << tau->Vt() << " ]";
  LOG("ComputeAttenuation", pDEBUG) << "  Direction  = [ " << tau->Px()/tau->E() << " , " << tau->Py()/tau->E() << " , " << tau->Pz()/tau->E() << " ]";
  ...
  double d_r = (gOptEnableDecayLength) ? -TMath::Log( rnd->RndGen().Rndm() ) * Tauola::tau_lifetime * 1e-3 : 0; //lifetime from mm(tauola) to m
  double vx = tau->Vx() + tau->Px()*d_r/Tauola::getTauMass();
  double vy = tau->Vy() + tau->Py()*d_r/Tauola::getTauMass();
  double vz = tau->Vz() + tau->Pz()*d_r/Tauola::getTauMass();
  double t  = tau->Vt() + tau-> E()*d_r/Tauola::getTauMass()/constants::kLightSpeed/(units::meter/units::second);

  for ( int sec=1; sec<Tauola_evt->getParticleCount(); sec++ ) {
    int pdg   = Tauola_evt->getParticle(sec)->getPdgID();
    double e  = Tauola_evt->getParticle(sec)->getE();
    if ( pdg::IsNeutrino(TMath::Abs(pdg)) && e>gOptEmin ) {
      double px = Tauola_evt->getParticle(sec)->getPx();
      double py = Tauola_evt->getParticle(sec)->getPy();
      double pz = Tauola_evt->getParticle(sec)->getPz();
      LOG("ComputeAttenuation", pDEBUG) << "Product: " << pdg << ", E = " << e;
      LOG("ComputeAttenuation", pDEBUG) << "  Position   = [ " << vx << " , " << vy << " , " << vz << " , " << t << " ]";
      LOG("ComputeAttenuation", pDEBUG) << "  Direction  = [ " << px/e << " , " << py/e << " , " << pz/e << " ]";
      Prod.push_back(GHepParticle(pdg,kIStUndefined,-1,-1,-1,-1,px,py,pz,e,vx,vy,vz,t));
    }
  }
...
void GetCommandLineArgs(int argc, char ** argv)
{
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg.compare(0,1,"-")==0){
      ...
      if(opt.compare("-r")==0){   
        i++;
        LOG("ComputeAttenuation", pINFO) << "Reading neutrino source radius";
