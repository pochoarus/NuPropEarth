*! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ !
*! TAUSIC: TAU-lepton SImulation Code (for standard rock)
...
*! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ !
      ...
      subroutine initialize_tausic_sr(ic)
      ...
      COMMON/sig_tau_sr/v2,cs1,cs2,j0
      COMMON/coenlo_tau_sr/emulo,emc
      COMMON/ang_tau_sr/ema,va,anga,dang
      COMMON/rock2_tau_sr/n_a,n_z,n_rho,n_lambda

* Rock description
      n_z=11.00     !Z, 11.00 for standard rock, 6.6 for pure water
      n_a=22.00     !A, 22.00 for standard rock, 11.89 for pure water
      n_rho=2.65     !density, 2.65 for standard rock
      n_lambda=26.48 !rad. length, 26.48 for standard rock
      rho=n_rho

*       reading of the muon energy losses
*      PRINT *,'tausic: loading energy losses table...'
      if(ic.eq.1) then
      OPEN(UNIT=4,file='tausic-eloss-sr-bs-2004.dat',
     *form='formatted',status='old')
      end if
      if(ic.eq.2) then
      OPEN(UNIT=4,file='tausic-eloss-sr-f2-2004.dat',
     *form='formatted',status='old')
      end if
      READ(4,99)emulo
...
*      PRINT *,'tausic: loading inel. scatt. cross section...'
      if(ic.eq.1) then
      OPEN(UNIT=3,file='tausic-double-diff-rock.dat',
     *  FORM='formatted',STATUS='old')
      end if
      if(ic.eq.2) then
      OPEN(UNIT=3,file='tausic-double-diff-rock-f2.dat',
     *  FORM='formatted',STATUS='old')
      end if
...
*      PRINT *,'tausic: loading integral cross sections...'
      if(ic.eq.1) then
      OPEN(UNIT=1,file='tausic-cross-sections-sr-bs-2004.dat',
     *  FORM='formatted',STATUS='old')
      end if
      if(ic.eq.2) then
      OPEN(UNIT=1,file='tausic-cross-sections-sr-f2-2004.dat',
     *  FORM='formatted',STATUS='old')
      end if
      READ(1,3)CS2
...
c        end if
        ...
        CALL tausic_sr(emu,depth,emu_f,x,y,z,t,theta,phi,dr,
     *       path_max,ms_flag,d_flag,idec,taudec,ttau)
        ...
        call coord_transform_tau_sr(x1,y1,z1,x,y,z,thetac,phic)
        xf=x1/n_rho+x0
        yf=y1/n_rho+y0
        zf=z1/n_rho+z0
        theta0=thetac
        phi0=phic
        call angle_transform_tau_sr(theta0,phi0,theta,phi)
        czf=dcos(theta0)
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      SUBROUTINE TAUSIC_SR(Emu,zf,Emu_f,x,y,z,t0,theta,phi,dr,t10,
...
C       DUE TO IONISATION AND OTHER PROCESSES FOR V=E/EMU<10**(-3)
      ...
      CALL vdem_tau_sr(em1,B0,BS)
      emf=em1-(em1*BS+B0)*T2
      IF(emf.LE.mmu) emf=mmu
      emi=10.**((dlog10(emf)+dlog10(em1))/2.)
      CALL vdem_tau_sr(emi,b0,bs)
      CALL vem_tau_sr(em1,b0,bs,t2,em3)
      IF(em3.LE.mmu) THEN
        dr=t1-t0+em1/b0
        em1=mmu
        go to 16
...
*       transformation of the coordinates
      CALL coord_transform_tau_sr
     *     (dx1,dy1,dz1,deltax,deltay,dz,theta,phi)
*       transformation of the angles
      IF(theta2.GT.0.) THEN
        CALL angle_transform_tau_sr(theta,phi,theta2,phi2)
      END IF
...
*       interaction point
      ...
      emi=10.**((dlog10(emf)+dlog10(em1))/2.)
      CALL vdem_tau_sr(emi,B0,BS)
      CALL vem_tau_sr(em1,B0,BS,T,EM2)
      IF(em2.LE.mmu) THEN
        dr=t1-t0+em1/b0
        em1=mmu
        go to 16
      ...
      IF(ms_flag.EQ.1) THEN
        CALL multiple_tau_sr           !multiple scattering between 2 interactions
...
c       simulation of the deflection angle of muon
      ...
      IF(ip.LE.3.AND.ip.GE.1.AND.d_flag.EQ.1) THEN
        CALL mu_scatt_tau_sr(ip,em,v1,theta1,phi1)
      END IF
...
c       transformation of the deflection angles to the base reference frame
      IF(theta1.GT.0.) THEN
        CALL angle_transform_tau_sr(theta,phi,theta1,phi1)
      END IF
...
C       LOSSES ARE CALCULATED
      ...
      emi=10.**((dlog10(emf)+dlog10(em1))/2.)
      CALL vdem_tau_sr(emi,b0,bs)
      CALL vem_tau_sr(em1,b0,bs,t4,em3)
      IF(em3.LE.mmu) THEN
         dr=t1-t0+em1/b0
         em1=mmu
         go to 16
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      SUBROUTINE vcs_tau_sr(j,yfl,ip,v1)
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      SUBROUTINE vdem_tau_sr(EM,B0,BS)
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      SUBROUTINE vem_tau_sr(EM0,B0,BS,T,EM)
C       CALCULATION OF THE CONTINUOUS ENERGY LOSSES
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      implicit real*8 (a-h,o-z)
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      SUBROUTINE defl_tau_sr(em,ip,v,theta,phi)
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      SUBROUTINE mu_scatt_tau_sr(jp,E,v,theta,phi)
      IMPLICIT NONE
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
* Input:
      INTEGER*4 jp
...
* Inelastic scattering
      ...
      ELSE IF (jp.EQ.3) THEN
        CALL defl_tau_sr(E,jp,v,theta,phi)
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	SUBROUTINE coord_transform_tau_sr(x,y,z,x1,y1,z1,theta1,phi1)
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	IMPLICIT real*8 (a-h,o-z)
	parameter (pi=3.141592654d0)	
	phi2=phi1
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	SUBROUTINE angle_transform_tau_sr(theta,phi,theta1,phi1)
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	IMPLICIT real*8 (a-h,o-z)
	parameter (pi=3.141592654d0)
